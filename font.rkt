...
(provide (all-defined-out)
         (rename-out [font-info? font?]))

;; ----------------------------------------------------

(struct font-info [base advance height sprites])
...
(define (make-font sprites #:advance [advance 8] #:base [base 33] #:height [height #f])
  (let* ([chars (for/vector ([sprite sprites]) sprite)]
         [line-height (or height (if (vector-empty? chars)
                                     0
                                     (+ (sequence-length (vector-ref sprites 0)) 1)))])

    ; create the font-info
    (font-info base advance line-height chars)))

;; ----------------------------------------------------

(define (font-advance)
  (font-info-advance (font)))

;; ----------------------------------------------------

(define (font-height)
  (font-info-height (font)))

;; ----------------------------------------------------

(define (font-sprite char)
  (let* ([base (font-info-base (font))]
         [sprites (font-info-sprites (font))]

         ; ordinal sprite index of character to render
         [n (- (char->integer char) base)])
...
(define tall-font
  (make-font #:advance 6
             #((#x20 #x20 #x20 #x20 #x20 #x00 #x20 #x00)    ; !
